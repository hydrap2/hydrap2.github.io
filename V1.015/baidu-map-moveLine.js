...
var MoveLine = function MoveLine(map, userOptions) {
    ...
    // var drawMoveFor =function drawMoveFor(_this,markLines,animationCtx) {
    ...
    //         markLine =null
    //         if(i==markLines.length-1){
    //             drawMoveFor(_this,markLines,animationCtx);
    //         }
    //     }
    // };
    //鼠标事件
    ...
    var addMarkLine = function addMarkLine() {
        ...
        dataset.forEach(function (line, i) {
            markLines.push(new MarkLine({
                ...
                from: new Marker({
                    // city: line.from.city,
                    location: new BMap.Point(line[0].lng, line[0].lat),
                    color: options.colors[i]
                }),
                to: new Marker({
                    // city: line.to.city,
                    location: new BMap.Point(line[1].lng, line[1].lat),
                    color: options.colors[i]
                })
            }));
        });
